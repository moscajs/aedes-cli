...
async function start (program) {
  ...
  broker.on('subscribe', function (subscriptions, client) {
    logger.info('MQTT client \x1b[32m%s\x1b[0m subscribed to: %s, broker %s', client ? client.id : client, subscriptions.map(s => s.topic).join('\n'), broker.id)
  })

  broker.on('unsubscribe', function (subscriptions, client) {
    logger.info('MQTT client \x1b[32m%s\x1b[0m unsubscribed to: %s, broker %s', client ? client.id : client, subscriptions.map(s => s.topic).join('\n'), broker.id)
  })

  // fired when a client connects
  broker.on('client', function (client) {
    logger.info('Client Connected: \x1b[33m%s\x1b[0m, broker %s', (client ? client.id : client), broker.id)
  })

  // emitted when the client has received all its offline messages and be initialized
  broker.on('clientReady', function (client) {
    logger.info('Client ready: \x1b[33m%s\x1b[0m, broker %s', (client ? client.id : client), broker.id)
  })
  ...
  broker.on('clientDisconnect', function (client) {
    logger.info('Client Disconnected: \x1b[33m%s\x1b[0m, broker %s', (client ? client.id : client), broker.id)
  })

  // fired when a message is published
  broker.on('publish', function (packet, client) {
    logger.info('Client \x1b[31m%s\x1b[0m has published %s on %s, broker %s', (client ? client.id : 'BROKER_' + broker.id), packet.payload.toString(), packet.topic, broker.id)
  })
