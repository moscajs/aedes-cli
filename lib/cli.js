...
yargs
  ...
  .option('host', {
    ...
    type: 'string',
    default: process.env.IS_DOCKER ? '0.0.0.0' : '127.0.0.1'
  })
  .option('protos', {
...
async function start (program) {
  ...
  // BROKER ------------
  const { persistence, mq } = await initPersistences(config)
  ...
  return { servers, broker, persistence, logger }
}
