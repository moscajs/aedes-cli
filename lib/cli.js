...
function start (program, callback) {
  return function () {
    ...
    opts.id = program.brokerId || 'aedes-cli'

    var server = aedes(opts)

    server.on('subscribe', function (subscriptions, client) {
      console.log('MQTT client \x1b[32m%s\x1b[0m subscribed to: %s from broker %s', client ? client.id : client, subscriptions.map(s => s.topic).join('\n'), server.id)
    })

    server.on('unsubscribe', function (subscriptions, client) {
      console.log('MQTT client \x1b[32m%s\x1b[0m unsubscribed to: %s from broker %s', client ? client.id : client, subscriptions.map(s => s.topic).join('\n'), server.id)
    })

    // fired when a client connects
    server.on('client', function (client) {
      console.log('Client Connected: \x1b[33m%s\x1b[0m to broker %s', (client ? client.id : client), server.id)
    })

    // fired when a client disconnects
    server.on('clientDisconnect', function (client) {
      console.log('Client Disconnected: \x1b[33m%s\x1b[0m to broker %s', (client ? client.id : client), server.id)
    })

    // fired when a message is published
    server.on('publish', async function (packet, client) {
      console.log('Client \x1b[31m%s\x1b[0m has published %s on %s to broker %s', (client ? client.id : 'BROKER_' + server.id), packet.payload.toString(), packet.topic, server.id)
    })
    ...
    var ports = {
      tcp: program.port || 1883,
      ws: program.wsPort || 3000,
      tls: program.tlsPort || 8883,
      wss: program.wssPort || 4000
    }

    for (const p of program.protos) {
      series.push(createServer.bind(this, p, program.host || '127.0.0.1', ports[p], secureOpts, server.handle))
    }
