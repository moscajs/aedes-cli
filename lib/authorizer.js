...
Authorizer.prototype.authorizePublish = function () {
  ...
  return function (client, topic, payload, cb) {
    cb(null, minimatch(topic, that.users.get(client.user).authorizePublish || defaultGlob))
  }
}
...
Authorizer.prototype.authorizeSubscribe = function () {
  ...
  return function (client, topic, cb) {
    cb(null, minimatch(topic, that.users.get(client.user).authorizeSubscribe || defaultGlob))
  }
}
...
Authorizer.prototype._authenticate = function (client, user, pass, cb) {
  var missingUser = !user || !pass || !this.users.get(user)
...
/**
 * An utility function to set users
 *
 * @api public
 * @param {Object} users Users object where username is the key
 */
Authorizer.prototype.setUsers = function (users) {
  users = users || {}
  this.users = new Map(Object.entries(users))
}

/**
 * An utility function to serialize users map
 *
 * @api public
 */
Authorizer.prototype.serialize = function () {
  return mapToObj(this.users)
}
